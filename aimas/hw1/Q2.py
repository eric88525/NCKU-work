...
'''
# given signal , return peaks count
def count_peaks(signal , threshold=1.5 ):

    signal_std = np.std(signal)
    signal_mean = np.mean(signal)

    div_signal = (signal >= (signal_mean+signal_std*threshold)).astype(np.int32)

    peaks = 0

    for i in range(0,div_signal.shape[0]-1):
        if div_signal[i] == 0 and div_signal[i+1] == 1:
            peaks += 1
