...
def main():
    x_list, y_list = hookjeeve_method([-5, 5], [-5, 5], himmelblau_func)
    ...
    plt.title(
        f"Hooke-Jeeves: f({x_list[-1][0]:.3f}, {x_list[-1][1]:.3f}) = {himmelblau_func(x_list[-1][0], x_list[-1][1]):.3f}")
    plt.savefig('Hooke-Jeeves.png')
    plt.show()
